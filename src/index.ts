...
export interface ResolvedInputs {
  ...
  monitorId: string;
  mockUrl: string;
  mockVisibility: RequestedMockVisibility;
  ...
  /** Cloud secret store the generated environments seed credential slots for. */
...
function resolveExplicitMock(
  mocks: MockRecord[],
  explicitUrl: string,
  collectionUid: string,
  environmentUid: string,
  requestedVisibility: RequestedMockVisibility
): MockRecord {
...
function parseOnboardingScope(value: string): 'full' | 'spec-only' {
  ...
  if (definition.allowedValues?.includes(resolved)) {
    return resolved as 'full' | 'spec-only';
  }
  throw new Error(
...
function parseMockVisibility(value: string | undefined): RequestedMockVisibility {
  ...
  if (allowed.includes(normalized)) return normalized as RequestedMockVisibility;
  throw new Error(
...
async function runRepoSyncInner(
  ...
  const versionRequested =
    ...
    (isFullOnboarding && inputs.collectionSyncMode === 'version');
  ...
  const preparedPrebuiltCollections = isFullOnboarding
  ...
  const envUids = isFullOnboarding
  ...
  if (
    ...
    if (mockEnvUid) {
      let resolvedMockUrl = '';
      let resolvedMockVisibility: RequestedMockVisibility | '' = '';
      ...
      if (inputs.mockUrl) {
        ...
        resolvedMockVisibility = resolved.visibility as RequestedMockVisibility;
